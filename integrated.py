...
def rise_experiment(days, total_assets, init_invest_ratio, win_ratio, lose_ratio, buy_ratio):
	...
	for times in range(n):
		...
		for day in range(days):
			for i in range(stock_count):
				if rr[i][day] > fall_rate: # rise
					current_price[i].append(current_price[i][-1]+current_price[i][0]*(rise_rate*(rr[i][day]-fall_prob)))
				else: #fall
					current_price[i].append(current_price[i][-1]+current_price[i][0]*(fall_rate*(rr[i][day]-fall_prob)))
				if current_price[i][-1] < 0.1: # bankrupt
					# bankrupt_count += 1
					# stock_num[i] = 0
					current_price[i][-1] = 0.1
		...
		# plt.show()
		# print("bankrupt count: ", bankrupt_count)
...
def fall_experiment(days, total_assets, init_invest_ratio, win_ratio, lose_ratio, buy_ratio):
...
for win_ratio in np.arange(0.1, 0.6, 0.1):
	...
	for lose_ratio in np.arange(0.1, 0.6, 0.1):
		round(lose_ratio, 1)
		for buy_ratio in np.arange(0.5, 3.5, 0.5):
			round(buy_ratio, 1)
			rise_final_assets, rise_avg_trend = rise_experiment(300, 1e9, 0.001, win_ratio, lose_ratio, buy_ratio)
			rise_win_wrt_trend = rise_final_assets/rise_avg_trend
			fall_final_assets, fall_avg_trend = fall_experiment(300, 1e9, 0.001, win_ratio, lose_ratio, buy_ratio)
			fall_win_wrt_trend = fall_final_assets/fall_avg_trend
			rise_fall_ratio = rise_win_wrt_trend/fall_win_wrt_trend

			if rise_fall_ratio > best_param[0]:
				best_param = [rise_fall_ratio, rise_final_assets, rise_avg_trend, fall_final_assets, fall_avg_trend, win_ratio, lose_ratio, buy_ratio]
			count += 1
			if count == limit:
				break
		if count == limit:
...
print("best param: ")
print("rise fall ratio ", best_param[0])
print("rise final assets: ", best_param[1])
print("rise avg trend: ", best_param[2])
print("fall final assets: ", best_param[3])
print("fall avg trend: ", best_param[4])
print("win ratio: ", best_param[5])
print("lose ratio: ", best_param[6])
print("buy ratio: ", best_param[7])
print("")
